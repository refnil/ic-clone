...
village1 = toDef ( [ (an "general store", Action (S.singleton Talking) "Talk to the general store employee." (anc "open door") 5)
...
actionSuite name count skills text condition costGenerator = fmap genAction [0..count]
    where genAction i = (ActionName name i, Action skills text (if i == 0 then condition else ActionCondition (ActionName name (i-1))) (int2Float $ costGenerator i))
